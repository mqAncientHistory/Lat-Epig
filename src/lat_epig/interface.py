...
def makeScrapeInterface():
    ...
    term1 = widgets.Text(description="Text 1:")
    operator = widgets.ToggleButtons(
        options=Parseargs.operatorlist, value="and", description="Operator"
    )

    term2 = widgets.Text(description="Text 2:")

    EDCS = widgets.Text(description="EDCS-ID:")

    publication = widgets.Text(description="Publication:")

    province = widgets.SelectMultiple(
        options=Parseargs.provincelist, rows=10, description="Province:"
    )
    place = widgets.Text(description="Place:")

    dating_from = widgets.Text(description="Dating from:")

    dating_to = widgets.Text(description="Dating to:")
    ...
    and_not_inscription_genus = widgets.SelectMultiple(
        options=Parseargs.genus_list, rows=10, description="and not:"
    )
    ...
    def on_button_clicked(b):
        with out:
            ...
            display(
                HTML(
                    "<p>Getting the inscriptions. This may take a few minutes (or hours), depending on the number of search results. To get ca. 1000 results take usually less than 5 minutes. Go get a nice cup of coffee!</p>"
                )
            )
            ...
            for output in OUTPUTS.glob("*.tsv"):                
                # check to see if there is a file that shares the same filename, but also a .json extesnion instead of .tsv
                json_file = OUTPUTS / f"{output.stem}.json"
                if not json_file.exists():
                    json_file = None

                file_outputs[f"{output.stat().st_mtime}{output.name}"] = (
                    output.name,
                    output,
                    json_file
                )
            # for output in OUTPUTS.glob("*.json"):
            #     json_file_outputs[f"{output.stat().st_mtime}{output.name}"] = (
            #         output.name,
            #         output,
            #     )
            ...
            for zipfile in filenames:
                ...
                json_href = escape(str(zipfile[2]))
                print(tsv, json_href)
                if json_href:
                    json_string = f"(<a href=\"{json_href}\">JSON</a>)"
                else:
                    json_string = ""
                display(
                    HTML(
                        f"<li><a href='{tsv}'>{zipfile[0]}</a> {json_string} </li>"
                    )
                )
            display(HTML("</ul>"))
