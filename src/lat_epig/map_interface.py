...
def make_map_interface():
    ...
    map_button = widgets.Button(
        layout={"width": "max-content"}, description="Export Desired Map!"
    )
    ...
    map_refresh = widgets.Button(
        description="Update Data File List", layout={"width": "max-content"}
    )
    ...
    map_data = widgets.Dropdown(
        description="Data File", options=get_outputs(), layout={"width": "max-content"}
    )

    map_title = widgets.Text(description="Map Title:")
    ...
    map_show_provinces = widgets.RadioButtons(
        options=[("All Provinces", True), ("Provinces near inscriptions", False)],
        value=True,
        layout={"width": "max-content"},
    ...
    map_filetype = widgets.RadioButtons(
        options=["PNG", "TIFF", "EPS", "SVG", "PDF", "PDF with inscriptions"],
        value="PNG",
        layout={"width": "max-content"},
    ...
    map_will_cite = widgets.RadioButtons(
        options=[
            (
                "I will cite this software in the research I am producing, please, remove the large credit line from the map.",
                True,
            ),
    ...
    map_dimensions = widgets.RadioButtons(
        ...
        layout={"width": "max-content"},
        description="Map dimensions (PDF Mode only)",
    )

    display(
        HTML("<h1>Export Map</h1>"),
        map_refresh,
        map_data,
        map_title,
        map_shapefile,
        map_show_provinces,
        map_basemap_multicolour,
        map_greyscale,
        map_show_roads,
        map_show_cities,
        map_filetype,
    ...
    def map_on_button_clicked(b):
        ...
        with out:
            ...
            for zipfile in filenames:
                display(
                    HTML(
                        f"<li><a target='_blank' href='{zipfile[1]}'>{zipfile[0]}</a></li>"
                    )
                )
            display(HTML("</ul>"))
