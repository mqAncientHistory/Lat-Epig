...
def make_map(
    ...
    provinces_3857 = geopandas.read_file(PROVINCES_SHP / province_shapefilename).to_crs(
        epsg=3857
    )
    ...
    cities_geodataframe_3857 = geopandas.GeoDataFrame(
        cities_dataframe,
        geometry=geopandas.points_from_xy(
            cities_dataframe["Longitude (X)"], cities_dataframe["Latitude (Y)"]
        ),
        crs="EPSG:4326",
    ).to_crs(epsg=3857)
    ...
    # https://geopandas.org/gallery/plotting_with_geoplot.html
    # world = geopandas.read_file("ne_110m_land.geojson")
    # ax = geoplot.polyplot(
    #     world,
    ...
    for key, val in import_metadata.items():
        if type(val) == str:
...
<http://awmc.unc.edu/wordpress/map-files/>"""
    ...
    buffer = geopandas.GeoDataFrame(
        geometry=point_dataframe_3857.buffer(MIN_MAP, cap_style=3)
    )
    ...
    province_shapefilename = (
        province_shapefilename.replace("_provinces.shp", "")
        .replace("ad", "AD")
        .replace("bc", "BC")
        .replace("roman_", "")
        .replace("empire_", "")
    ...
    if show_ids:
        for x, y, label in zip(
            point_dataframe_3857.geometry.x,
            point_dataframe_3857.geometry.y,
            point_dataframe_3857["EDCS-ID"],
        ):
            # print(x,y,label)
            ax.annotate(
                label,
    ...
    province_shapefilename = province_shapefilename.replace(" ", "_").replace(
        ".shp", ""
    )
    ...
    if filetype != "PDF":
        ...
        with PdfPages(
            ...
            if append_inscriptions:
                ...
                point_dataframe_3857["geom_4326"] = point_dataframe_3857[
                    "geometry"
                ].to_crs("EPSG:4326")
                cell_text = (
                ...
                for y_bucket in sorted(formatted_text):
                    for x_bucket in sorted(formatted_text[y_bucket]):
                        formatted_slice.append(
                            "\nInscriptions near Long: {}, Lat: {}".format(
                                y_bucket, x_bucket
                            )
                        )
