...
def makeDataframe(data_file, epsg=3857):
    ...
    ].apply(lambda x: textwrap.shorten(x or "null", width=255))
...
def make_interactive_map(data_file):
    ...
    roads = folium.features.GeoJson(
        roads_4326.to_json(), name="Roads", style_function=lambda x: road_style
    ).add_to(scrape_map)
...
"""
    ...
    provinces = folium.features.GeoJson(
        provinces_4326.to_json(), style_function=province_style
    ).add_to(scrape_map)
