...
from lat_epig.text_parse import (
    clean_conservative_rules,
    clean_interpretive_rules,
    clean,
)
import json
...
def scrape(args, prevent_write=False, show_inscription_transform=False):
    ...
    if "term1" in args.__dict__ and "%" not in args.__dict__["term1"]:
        searchString.append(
            "{}_{}".format(FILENAME_TERM["term1"], args.__dict__["term1"])
        )

    if "operator" in args.__dict__ and args.__dict__["operator"] != "and":
        searchString.append(
            "{}_{}".format(FILENAME_TERM["operator"], args.__dict__["operator"])
        )

    if (
        "term2" in args.__dict__
        and args.__dict__["term2"]
        and "%" not in args.__dict__["term2"]
    ):
        searchString.append(
            "{}_{}".format(FILENAME_TERM["term2"], args.__dict__["term2"])
        )
            ...
            if k != "debug" and k != "term1" and k != "term2" and k != "operator":
                ...
                if type(args.__dict__[k]) == str:
                    ...
                    if len(args.__dict__[k]) > 2:
                        ...
                        results = ",".join(words)
                        arg_str = "{}_{}({})".format(
                            FILENAME_TERM[k], len(args.__dict__[k]), results
                        )
                    else:
    ...
    def parseItem(result):
        ...
        for r in result:
            ...
            if type(r) == bs4.element.Tag:
                ...
                if maplink:
                    ...
                    longitude = re.search(
                        "longitude='([-0-9.]+)'", str(maplink["href"])
                    )
                ...
                if pubPhoto:
                    ...
                    item["photo"] = "http://db.edcs.eu/epigr/bilder.php?{}".format(
                        pubPhoto["href"]
                    )
                    pubPhoto.extract()
        ...
        if not item.get("publication"):
            html = re.sub(
                r"<b>publication:</b>[ \n]*(.*)<b>dating",
                r"<div class='publication'>\1</div><b>dating",
                html,
            )
            html = re.sub(
                r"<b>publication:</b>[ \n]*(.*)<b>EDCS",
                r"<div class='publication'>\1</div><b>EDCS",
                html,
            )
        else:
            html = re.sub(r"<b>publication:</b>", r"", html)
        html = re.sub(
            r"<b>dating:</b>(.*)<b>EDCS-ID",
            r"<div class='raw_dating'>\1</div><b>EDCS-ID",
            html,
        )
        html = re.sub(
        ...
        html = re.sub(
            r"<b>province:</b>([^<]*)", r"<div class='province'>\1</div>", html
        )

        html = re.sub(
            r"<b>place:</b>[ \n]*(.*)</noscript>",
            r"<div class='place'>\1</noscript></div>",
            html,
        )
        html = re.sub(
        ...
        html = re.sub(
            r"<b>inscription genus / personal status:</b>([^<]*)",
            r"<div class='status'>\1</div>",
            html,
        )
        html = re.sub(
            r"<b>material:</b>([^<]*)", r"<div class='material'>\1</div>", html
        )
        html = re.sub(
            r"<b>comment:</b>(.*)</p>", r"<div class='comment'>\1</div>", html
        )
        ...
        # dating ID: 72200182
        item["dating_from"] = item["dating_to"] = item["date_not_before"] = item[
            "date_not_after"
        ] = None
        ...
        if type(item["inscription"]) == str and re.search(
            language_pattern, item["inscription"]
        ):
            item["inscription"] = re.sub(
                language_pattern, pop_language, item["inscription"]
            )

        opening_quote_language_pattern = r"\"([A-Z]+)(?:\"|\b)"
        if type(item["inscription"]) == str and re.search(
            opening_quote_language_pattern, item["inscription"]
        ):
            item["inscription"] = re.sub(
                opening_quote_language_pattern, pop_language, item["inscription"]
            )

        closing_quote_language_pattern = r"(?:\"|\b)([A-Z]+)\""
        if type(item["inscription"]) == str and re.search(
            closing_quote_language_pattern, item["inscription"]
        ):
            item["inscription"] = re.sub(
                closing_quote_language_pattern, pop_language, item["inscription"]
            )
        ...
        if item["inscription"]:
            ...
            item["inscription_conservative_cleaning"] = clean(
            ...
            item["inscription_interpretive_cleaning"] = clean(
                text=item["inscription"],
                mode="interpretive",
    ...
    inscriptionsFound = resultsSoup.findAll(
        string=re.compile("inscriptions found[^0-9]+([0-9]+)")
    )
    [inscription.parent.parent.extract() for inscription in inscriptionsFound]

    # print(resultsSoup)

    inscriptions = re.search(
        "inscriptions found[^0-9]+0([0-9]+)", str(inscriptionsFound)
    ).group(1)
    ...
    # output = filter()
    basefilename = "{}-EDCS_via_Lat_Epig-{}-{}".format(
    ...
    if not prevent_write:
        with codecs.open(
            os.path.join("output", f"{basefilename}.json"), "w", encoding="utf-8"
        ) as jsonfile:
            data = {
        ...
        with codecs.open(
            os.path.join("output", f"{basefilename}.tsv"), "w", encoding="utf-8"
        ) as tsvfile:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Scraping of http://db.edcs.eu/epigr/epi.php?s_sprache=en"
    )
    ...
    parser.add_argument(
        ...
        "--publication",
        action="append",
        help="to include more than one publication, use -p more than once",
    )
    parser.add_argument(
        "-v",
        "--province",
        action="append",
        help="To include more than one province, use -v more than once",
    )
    ...
    parser.add_argument(
        ...
        "--operator",
        default="and",
        help="Default: and. Term Operator: and, or, not",
        choices=["and", "or", "not"],
    )
    ...
    parser.add_argument(
        ...
        "--inscription-genus",
        action="append",
        help="To include more than one genus, -ig more than once.",
    )
    parser.add_argument(
        "-ng",
        "--and-not-inscription-genus",
        action="append",
        help="To exclude more than one genus, -ng more than once.",
    )
    parser.add_argument(
        "--to-file",
        help="save the search results as a webpage on the machine. Debug tool",
    )
    parser.add_argument(
        "--from-file",
        help="use the saved webpage instead of going to the manfred claus server. Debug.",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Add the primary result debug column. Default (false)",
    )

    parser.add_argument(
        "term1",
        help='Search term, no flag is required. \n For phrases wrap them in "". For example, a one word search: platea. \nFor example, to search for Caesar divi Nervae, you write: ./parse.py "Caesar divi Nervae" to have Caesar... in the first search term. To have sophisticated wildcard matching (For example Caesar (anything) Nervae), ask Brian, or look at http://db.edcs.eu/epigr/hinweise/hinweis-en.html',
    )
